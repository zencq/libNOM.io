...
public class SteamTest : CommonTestClass
{
    ...
    private static void AssertSpecificMeta(WriteResults results, IContainer containerA, IContainer containerB, uint[] metaA, uint[] metaB)
    {
        ...
        AssertAllAreEqual(results.TotalPlayTime, containerA.TotalPlayTime, containerB.TotalPlayTime, metaA[19], metaB[19]); // metaA and metaB are actual uint but the values we compare are low enough to not require a ulong
